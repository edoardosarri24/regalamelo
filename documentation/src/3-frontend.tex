\chapter{Frontend}
This chapter details the architectural decisions and technical specifications for the client-side application of the "Regalamelo" project. The frontend serves as the primary interface for both Celebrants (to manage lists) and Guests (to view and claim gifts).
...
\begin{itemize}
    ...
    \item Internationalization \\
        Custom \texttt{LanguageContext} using React Context API. To manage translations and user language preferences. The app is fully localized in Italian and English.
...
The application supports English and Italian using a custom \texttt{LanguageContext}.
\begin{itemize}
    \item \textbf{Detection}: The language is initially detected via the browser settings and persisted in \texttt{localStorage}.
    \item \textbf{Structure}: Translation strings are organized in a centralized \texttt{LanguageContext.tsx} file.
...
/src
  ...
  /features       # Domain-specific logic
    ...
    /lists        # List view, Item card, Claim logic
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{/verify-email}: Page that automatically processes the Magic Link \texttt{token} from the URL to activate the account.
    ...
    \begin{itemize}
        ...
        \item \texttt{/dashboard/:slug}: Management view for a specific list. Includes functionality to add, delete, and \textbf{edit} gift items (updating name, description, image, URL, and preference).
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textit{Regalamelo}: If a list has no items, Guests see a friendly message indicating that the celebrant is still working on it.
...
To ensure maximum visibility for the application, a comprehensive SEO strategy is implemented, balancing the nature of a Single Page Application (SPA) with search engine requirements.

\begin{itemize}
    \item \textbf{Static SEO Foundation} \\
        Basic indexing is supported through static files in the \texttt{public} directory:
        \begin{itemize}
            \item \texttt{robots.txt}: Defines crawler access rules.
            \item \texttt{sitemap.xml}: Lists public routes for faster indexing.
            \item \textbf{Meta Tags \& Open Graph}: The \texttt{index.html} includes foundational \texttt{<meta>} tags, canonical links, and Open Graph properties to ensure rich previews when links are shared on social platforms.
        \end{itemize}
    \item \textbf{Dynamic SEO with React Helmet Async} \\
        Since React applications do not reload the page on navigation, dynamic metadata injection is required. \texttt{react-helmet-async} is utilized to dynamically update the \texttt{<title>}, \texttt{<meta name="description">}, and other relevant tags based on the current component (e.g., setting the title to a specific list's name: "Marco's Gift List | Regalamelo").
    \item \textbf{Structured Data (JSON-LD)} \\
        For public-facing pages, JSON-LD scripts are injected to provide search engines with machine-readable context, increasing the likelihood of rich search results.
\end{itemize}
